...
p1 <- ggplot(data = demulti, aes(x = i7, y = i5), stat="identity") +
  ...
  theme(axis.text.x = element_text(angle=90, hjust=1), plot.title=element_text(hjust = 0.5), legend.position = "bottom") +labs(title= "100-500-1000 Mock Samples")
...
c5 <- demulti[demulti$i5 %in% combinatorial$index2 & demulti$i7 %in% combinatorial$index, ]
...
u5 <- demulti[demulti$i5 %in% unique$index2 & demulti$i7 %in% unique$index, ]
